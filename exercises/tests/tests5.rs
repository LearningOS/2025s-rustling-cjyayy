...
/// 1. The address is properly aligned for a `u32`
/// 2. The address points to a valid, initialized `u32` value
/// 3. There are no other references (mutable or immutable) to the same memory
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The caller has guaranteed that the address is valid,
    // properly aligned, and points to a unique mutable `u32`.
    // We can safely dereference this pointer to modify the value.
    unsafe {
